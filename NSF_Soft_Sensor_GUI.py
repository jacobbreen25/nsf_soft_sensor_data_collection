...
class NSF_Gui(tk.Tk):
    ...
    async def construct_gui(self):
        ...
        f.append(tk.Frame(master=self))
        bIdle = tk.Button(master=f[-1], text="Idle/Transition", command= lambda: Thread(setLabel(bIdle, "Idle")), state=tk.DISABLED)
        self.disabledButton = bIdle
        ...
        bIdle.pack(side=tk.LEFT, pady=5, padx=5)
    ...
    async def bluetoothDaemon(self):
        ...
        while(True):
            ...
            if (glb_command == "connect"):
                glb_command = str
                task = asyncio.create_task(self.connect_bluetooth("FlexibleSensorBLE-100"))
                await task
